...
export default function QuizPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    // Simuler le chargement d&apos;un quiz depuis le localStorage
    // Dans une implémentation réelle, vous récupéreriez les données depuis Firestore
    const fetchQuiz = async () => {
      try {
        setLoading(true);
        
        // Vérifier si le quiz est déjà dans le localStorage
        const cachedQuiz = localStorage.getItem(`quiz_${documentId}`);
        ...
        const response = await fetch('/api/ai/mock-quiz', {
          method: 'POST',
          headers: {
          ...
          body: JSON.stringify({
            documentId,
            text: "Exemple de texte mathématique pour générer un quiz",
        ...
        if (data.success && data.quiz) {
          ...
          localStorage.setItem(`quiz_${documentId}`, data.quiz);
          setQuiz(data.quiz);
        } else {
      ...
      <MainLayout>
        <div className="container mx-auto px-4 py-10">
          <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
            ...
            <h2 className="text-xl font-semibold text-red-700 mb-2">{error || "Quiz non disponible"}</h2>
            <p className="text-red-600 mb-4">Le quiz demandé n&apos;a pas pu être généré.</p>
    ...
    <MainLayout>
      <div className="container mx-auto px-4 py-10">
            ...
            <Button 
              ...
              onClick={() => {
                ...
                navigator.clipboard.writeText(quiz)
                  .then(() => {
                    alert("Quiz copié dans le presse-papier");
                  })
                  .catch(err => {
                    console.error("Erreur lors de la copie:", err);
                    alert("Impossible de copier le quiz");
                  });
              }}
            >
              Copier
        ...
        <div className="flex justify-between mt-8">
          ...
          <Button 
            onClick={() => router.push(`/document/${documentId}/explanation`)}
          >
            Voir l&apos;explication
          </Button>
        </div>
